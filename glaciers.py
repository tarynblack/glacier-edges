...
def Create_chips(vrt, glacier_info):
    '''
    ...
    for line in lines:
        ...
        subprocess.call(['gdalwarp','-t_srs','EPSG:3413','-te',glacier_coords[1],glacier_coords[2],glacier_coords[3],glacier_coords[4],'-tr','50','50',vrt,glacier_coords[0]+'.tif'])
    
for date in dated_directories:
    os.chdir(mosaics_path+date)
    path = os.getcwd()
    vrt = Get_vrt(path)
    Create_chips(vrt, glaciers_txt)
